...
public class ListAdapterView21mag extends BaseAdapter {
    Context mContext;
    LayoutInflater inflater;
    List<Model> modelList;
    ArrayList<Model> arrayList;

    public ListAdapterView21mag(Context context, List<Model> modelList){
    ...
    public View getView(int position, View view,ViewGroup parent) {
        ...
        view.setOnClickListener(new View.OnClickListener() {
            ...
            {
                if(modelList.get(position).getTitle().equals("2020"))
                {
                    //Start new activity
                    Intent intent=new Intent(mContext,CalendarActivity2.class);
                    intent.putExtra("actionBarTitle", "Kurukshetra");
                    //intent.putExtra("contentTv","this is sociology in detail");
                    mContext.startActivity(intent);
                }
                if(modelList.get(position).getTitle().equals("2019"))
                {
                    //Start new activity
                    Intent intent=new Intent(mContext,CalendarActivity2.class);
                    intent.putExtra("actionBarTitle", "Kurukshetra");
